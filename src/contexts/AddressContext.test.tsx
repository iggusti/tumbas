...
const TestComponent = () => {
  ...
  return (
    <div>
      ...
      <div data-testid="selected-address-label">{selectedAddress?.label || 'none'}</div>
      <button onClick={() => addAddress({
        ...
        isDefault: false,
        icon: 'other'
      })} data-testid="add-address">Add Address</button>
      <button onClick={() => updateAddress('1', { label: 'Updated Home' })} data-testid="update-address">Update Address</button>
      <button onClick={() => deleteAddress('2')} data-testid="delete-address">Delete Address</button>
      <button onClick={() => setDefaultAddress('2')} data-testid="set-default">Set Default</button>
      <button onClick={() => selectAddress('2')} data-testid="select-address">Select Address</button>
    </div>
  );
};
...
describe('AddressContext', () => {
  it('should provide initial addresses', () => {
    ...
    expect(screen.getByTestId('selected-address-label')).toHaveTextContent('Rumah');
  });
  ...
  it('should update address', () => {
    ...
    expect(screen.getByTestId('selected-address-label')).toHaveTextContent('Updated Home');
  });
