...
const TestComponent = () => {
  const { cartItems, addToCart, updateQuantity, removeItem, toggleCheck, clearCart, clearCheckedItems } = useCart();

  return (
    <div>
      <div data-testid="cart-count">{cartItems.length}</div>
      <button onClick={() => addToCart('test-product', 1)} data-testid="add-to-cart">Add to Cart</button>
      <button onClick={() => updateQuantity('1', 1)} data-testid="update-quantity">Update Quantity</button>
      <button onClick={() => removeItem('1')} data-testid="remove-item">Remove Item</button>
      <button onClick={() => toggleCheck('1')} data-testid="toggle-check">Toggle Check</button>
      <button onClick={() => clearCart()} data-testid="clear-cart">Clear Cart</button>
      <button onClick={() => clearCheckedItems()} data-testid="clear-checked">Clear Checked</button>
    </div>
  );
};
