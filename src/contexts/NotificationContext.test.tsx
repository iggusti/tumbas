...
const TestComponent = () => {
  const { notifications, addNotification, markAsRead, markAllAsRead, clearNotifications, hasUnreadNotifications } = useNotification();

  return (
    <div>
      <div data-testid="notifications-count">{notifications.length}</div>
      <div data-testid="has-unread">{hasUnreadNotifications() ? 'true' : 'false'}</div>
      <button onClick={() => addNotification({
        type: 'promo',
        title: 'Test Notification',
        description: 'Test description',
        link: '/test'
      })} data-testid="add-notification">Add Notification</button>
      ...
      <button onClick={() => clearNotifications()} data-testid="clear-notifications">Clear Notifications</button>
    </div>
  );
};
